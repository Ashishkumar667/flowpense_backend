...
try {
  ...
  if (Pagaresponsedata.responseCode === "0" || Pagaresponsedata.responseCode === 0 ) {
    ...
    return res.status(200).json({
      ...
      pagaResponse: Pagaresponsedata, //added paga response 
    });
  } else {
...
export const pagaWebhook = asyncHandler(async (req, res) => {
  try {
    ...
    const expectedHash = crypto
      .createHash("sha512")
      .update(data.statusCode + data.accountNumber + data.amount + data.clearingFeeAmount + process.env.PAGA_HMAC_SECRET)
      .digest("hex");
